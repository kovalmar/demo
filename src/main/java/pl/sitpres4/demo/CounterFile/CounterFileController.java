...
import pl.sitpres4.demo.CounterData.CounterDataFlexible;
...
public class CounterFileController {
    ...
    @GetMapping("/counters/{number}/files/{year}/{month}/dataflexible")
    public CounterDataFlexible counterDataFlexible(@PathVariable("number") String number,
                                                   @PathVariable("year") String year,
                                                   @PathVariable("month") String month) {
        //TODO
        return new CounterDataFlexible(String.format("%s%s_%s.CSV",
                DataFromFTP.getInstance().getCountersFromConfig().get(Integer.parseInt(number)).getFtpFileNameMask(),
                year,month));
    }
}
